...
import { useModuleData, useModuleDataOnly } from "./ModuleDataContext";

export default function ReportComponentNew(props){
    const {records} = useModuleData();
    ...
    return (<div className="reportContainer">
                ...
                <div className="reportBody">
                    ...
                    <ReportFilter fields={props.fields}></ReportFilter>

                    <div className="listContainer">
                       <List records={records} fields={props.fields} changeFields={changeFields} showPreview={showPreview} selectedRecordId={selectedRecord?selectedRecordId:null}></List> 
                    
                        <Pagination recordsLength={records.length}></Pagination>
                    </div>    
                    
                    {(selectedRecordId === "new") && <Form fields={props.fields} moduleDetail={moduleDetail} close={()=>setSelectedRecordId(null)}></Form>}
                    
                    {selectedRecord && <RecordView changeRecord={changeRecord} closeQuickView={()=>{setSelectedRecordId(null)}} record={selectedRecord} fields={props.fields} recordIndex={records.findIndex(re=>re.id === selectedRecordId)}></RecordView>}    
